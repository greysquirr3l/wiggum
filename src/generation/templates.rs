...
static TEMPLATES: LazyLock<Tera> = LazyLock::new(|| {
    ...
    let templates = [
        ...
        ("capability.md", CAPABILITY_TEMPLATE),
        // opencode variants
        ("orchestrator_opencode.md", ORCHESTRATOR_OPENCODE_TEMPLATE),
        ("orchestrator_root.md", ORCHESTRATOR_ROOT_TEMPLATE),
...
2. If PROGRESS.md does not exist, fail immediately — it should have been created.{% if has_capabilities %}
3. Read every file in `capabilities/` — these are the behavioural contracts the
   project must satisfy. Subagents may reference them by slug; the orchestrator
   is the one that has the full picture. The current plan declares
   {{ capability_count }} capability(ies).
{% endif %}
...
{% if linked_capabilities | length > 0 %}
## Implements

{% for cap in linked_capabilities %}
### Capability: [{{ cap.name }}](../capabilities/{{ cap.name }}.md) — {{ cap.title }}

{% if cap.requirements %}{% for req in cap.requirements %}- {{ req }}
{% endfor %}{% endif %}{% if cap.scenarios %}
**Scenarios this task must satisfy:**

{% for s in cap.scenarios %}- **{{ s.name }}** — WHEN `{{ s.when }}` THEN `{{ s.then }}`
{% endfor %}{% endif %}
{% endfor %}
{% endif %}
## Project Context
...
The project must satisfy these behavioural contracts. Each capability has its own
file under `capabilities/`; task files that declare `implements = [...]` referencing the capability slug link
to the relevant capability and inline its scenarios.

{% for cap in capabilities %}
### {{ cap.title }}

- **File**: [`capabilities/{{ cap.name }}.md`](capabilities/{{ cap.name }}.md)
- **Capability slug**: `{{ cap.name }}`
- **Requirements**: {{ cap.requirements | length }}
- **Scenarios**: {{ cap.scenario_count }}
{% if cap.description %}
...
description: Orchestrator for {{ project_name }} — drives subagents through every task in PROGRESS.md.
mode: primary
...
2. Find the next task that is `[ ]` and whose dependencies are all `[x]`.
3. **Check for a gate** — if the task file begins with a `⛔ GATE` banner, emit it verbatim
   and **stop**. The human must confirm before you proceed.
4. Mark it `[~]` in `PROGRESS.md`.
5. **Extract context for the subagent** — read `PROGRESS.md` and copy out the full text of
   the **Accumulated Learnings** section, the **Codebase State** section, and the
   **Completion Standard** section verbatim. You will inject this content directly
   into the subagent dispatch message in step 6.
6. Start a subagent via the `task` tool with `subagent_type: "general"`, **prepending the
   extracted Accumulated Learnings, Codebase State, and Completion Standard content at the
   top of the `prompt`** so the subagent receives it as live context, not a file reference.
   Use the `SUBAGENT_PROMPT` block at the bottom of this file as the body of the `prompt`.
7. Wait for the subagent to complete.
8. **Independently verify** — run the preflight yourself before trusting the subagent's `[x]`:
...
{% if has_evaluator %}
9. **Spawn the evaluator** — dispatch the `evaluator` subagent via `task` (see
   `.opencode/agents/evaluator.md`) with the task slug and goal. Wait for a PASS verdict
   before proceeding. If the evaluator returns FAIL, mark the task `[!]` and capture the
   evaluator's findings in `PROGRESS.md` for the next subagent iteration.
{% endif %}
10. **Spawn the background auditor** — dispatch the `background-auditor` subagent (see
    `.opencode/agents/background-auditor.md`) to check for cross-task regressions.
11. Read `PROGRESS.md` again.
12. Verify the task is now `[x]`. If it is not, mark it `[!]` and output a warning, then continue to the next available task.
13. Repeat.
...
Tasks in the same group have no intra-group dependencies and may be dispatched
to concurrent subagents. Run groups sequentially; within each group, launch all
tasks simultaneously using separate `task` tool calls (each with a unique
`description` and its own `prompt`).
...
You are a senior software engineer with deep familiarity with the project's rules and standards. Your job is to implement exactly one task from `PROGRESS.md`.
...
- Task files: `tasks/`
- Features registry: `features.json`

> The **Accumulated Learnings**, **Codebase State**, and **Completion Standard** from
> `PROGRESS.md` have been injected above by the orchestrator. Apply them before writing
> any code — Completion Standard is load-bearing, not flavour text.
...
5. **Sprint contract** — Before writing any code, state explicitly:
   ...
   - How you will verify each exit criterion listed in the task file
{% if contract_review %}
5a. **Contract review** — Dispatch the `evaluator` subagent (see
    `.opencode/agents/evaluator.md`) for contract review. Wait for APPROVED before
    starting implementation. Address any objections before proceeding.
{% endif %}
6. Implement the task completely — create all files, write all code, add all tests specified.
...
const ORCHESTRATOR_ROOT_TEMPLATE: &str = r#"# ORCHESTRATOR

`{{ project_name }}`'s implementation is driven by a multi-agent orchestration loop.
This document describes the workflow, the agents, the state machine, and the
...
## The plan, the tasks, the progress
...
| `features.json` | Per-task criteria registry. Evaluator writes `"passes": true` here on PASS. |
| `.opencode/agents/*.md` | Agent prompts (this project uses opencode). |
| `ORCHESTRATOR.md` | This file. Human-readable workflow reference. |
| `.vscode/*.prompt.md` | Equivalent prompts for VS Code Copilot agent mode. Both coexist. |

The TOML is the source. The other files are derived state. To change the plan,
edit the TOML and re-run `wiggum generate`.

## Agents

| Agent | Mode | Role |
|---|---|---|
| `orchestrator` | primary | Reads `PROGRESS.md`, finds the next ready task, dispatches an implementation subagent, runs preflight, dispatches the evaluator and background auditor, retries on failure. |
| `general` (built-in) | subagent | The implementation subagent. Spawned via the `task` tool with `subagent_type: "general"` and the inline `<SUBAGENT_PROMPT>` from the orchestrator prompt. |
| `evaluator` | subagent | Independent QA. Runs preflight, weights the rubric, flags hollow passes. Writes `features.json`. |
| `planner` | subagent | Decomposes goals into wiggum tasks. Used for *new* plan sections, not for day-to-day implementation. |
| `background-auditor` | subagent | Post-task cross-cutting audit — wiring, stub creep, dependency surface, API consistency, security surface. |

### Per-agent tooling

| Agent | read | edit | bash | task | webfetch |
|---|---|---|---|---|---|
| `orchestrator` | ✓ | ✓ | ✓ | ✓ | ask |
| `general` (subagent) | ✓ | ✓ | ✓ | — | — |
| `evaluator` | ✓ | ✓ (features.json only) | ✓ (preflight) | — | deny |
| `planner` | ✓ | ✓ | deny | — | deny |
| `background-auditor` | ✓ | deny (notes via orchestrator) | ✓ | — | deny |

The orchestrator is the only agent that writes `PROGRESS.md`. The evaluator
and background-auditor return structured findings; the orchestrator folds them
into the state file.

## Task state machine

```
   ┌──────┐    dispatch    ┌──────┐   preflight OK    ┌──────┐
   │ [ ]  │ ─────────────▶ │ [~]  │ ──── + eval ────▶ │ [x]  │
   │ idle │                │work  │                  │ done │
   └──────┘                └──────┘                  └──────┘
                              │                           ▲
                              │ eval FAIL / preflight FAIL│
                              ▼                           │
                           ┌──────┐    retry ≤ {{ max_retries }}         │
                           │ [~]  │ ─────────────────────┘
                           │retry │
...
| State | Meaning | Owner |
|---|---|---|
| `[ ]` | Not started; may have unfulfilled dependencies | none |
| `[~]` | Claimed by an in-flight subagent | one subagent |
| `[x]` | Done: preflight passed, evaluator PASS, audit clean | none |
| `[!]` | Blocked. Pre-flight or evaluator FAILed after retries; orchestrator paused. | human |

The orchestrator only dispatches a task when all its `depends_on` slugs are `[x]`.

## The implementation loop

For each `[ ]` task whose dependencies are `[x]`:

1. **Gate check.** If the task file begins with `⛔ GATE`, emit the banner verbatim and stop. The human must confirm.
2. **Claim.** Mark `[~]` in `PROGRESS.md`.
3. **Extract context.** Copy the full **Accumulated Learnings**, **Codebase State**, and **Completion Standard** sections out of `PROGRESS.md`.
4. **Dispatch implementation.** Spawn a `general` subagent via `task(subagent_type="general", prompt=<SUBAGENT_PROMPT with injected context>)`. The subagent implements and runs preflight.
5. **Independent preflight.** Orchestrator runs the same preflight itself. If it fails, the subagent's `[x]` claim is rejected — mark `[!]` and retry.
6. **Evaluator.** Spawn `evaluator` subagent. Weighted rubric must score ≥ {{ pass_threshold }}/10; hard-fail mode on.
7. **Background auditor.** Spawn `background-auditor` subagent. Findings are recorded in `PROGRESS.md` notes.
8. **Verify.** Confirm the task is `[x]` in `PROGRESS.md`. If not, mark `[!]` and continue to the next task (failures don't block unrelated tasks).
9. **Commit.** The implementation subagent commits per task. The orchestrator never commits on its own.

After {{ max_retries }} retries with preflight or evaluator failure, the orchestrator pauses
and emits a `⛔ Task T{NN} failed after {{ max_retries }} retries. Human intervention required.`
banner.
...
{% if has_evaluator %}This project uses a **weighted rubric**. Score each criterion from 0–10, then compute:
...
A weighted score ≥ {{ pass_threshold }} with **no criterion FAIL** is a PASS. Hard-fail mode is on — any single criterion failure is a FAIL regardless of score.{% else %}No `[evaluator]` section is configured in the plan TOML. The orchestrator runs preflight
itself to verify each task, but there is no independent QA pass. To enable an
evaluator, add an `[evaluator]` block to the plan TOML and re-run `wiggum generate`.{% endif %}

## Completion Standard

A standing directive on the bar for shipped work, owned by the project owner.
Edited rarely — typically once when the project is bootstrapped. The
orchestrator copies this section verbatim into every subagent dispatch so
that every fresh agent (including ones spun up after compaction) receives
the same load-bearing bar:
...
Two more persistent sections in `PROGRESS.md` travel with every subagent dispatch:

- **Accumulated Learnings** — bullet list of discoveries appended after each
  task. Format: `- T{NN}: {what you learned}`. Cross-task traps, dead-ends,
  non-obvious invariants. Read by the orchestrator before every dispatch so
  mistakes are not repeated.
- **Codebase State** — concise summary of what currently exists in the
  workspace after the last task. Updated after each `[x]`. A fresh agent
  should be able to orient from this section alone.

The orchestrator injects all three sections (Accumulated Learnings, Codebase
State, Completion Standard) verbatim into the implementation subagent's
`prompt` parameter on every dispatch.

## Gates

A task file may begin with a `⛔ GATE` banner. When the orchestrator encounters
one, it:

1. Emits the banner verbatim.
2. **Stops** — no further dispatch.
3. Waits for the human to restart the orchestrator.

Use gates sparingly: external service credentials, irreversible deployments,
schema migrations, anything where a wrong autonomous action costs more than a
human checkpoint.

## Session boundaries

When a context window ends mid-task (compaction or interrupt), the
orchestrator writes a `## Session handoff` section at the bottom of
`PROGRESS.md` with:

- Current task slug and status (`[~]`)
- Files modified so far
- Next concrete action needed

The orchestrator never marks a task `[x]` on the strength of a handoff alone —
on resume, the next iteration must independently verify preflight and the
evaluator must run.
...
```bash
# Pick the orchestrator agent in the opencode TUI, or invoke it directly:
opencode --agent orchestrator

# Or via slash command (auto-generated from agent name):
/orchestrator
```

The orchestrator auto-uses `.opencode/package.json` to install the
`@opencode-ai/plugin` runtime when needed.

### VS Code Copilot agent mode

Open `.vscode/orchestrator.prompt.md` in agent mode. The VS Code prompt set is
the dual of the opencode agent set; both implement the same workflow.
...
|---|---|---|
| Preflight fails on the lint command after a `[x]` claim | Implementation subagent added a lint suppression or loosened the profile | Reject the claim; mark `[!]`; retry with explicit instruction not to loosen. |
| Evaluator returns hollow-pass FAIL | `todo!()` / `unimplemented!()` left behind; tests assert nothing meaningful | Mark `[!]`; orchestrator captures finding; next iteration must remove stubs. |
| `PROGRESS.md` corrupted mid-loop | Concurrent edits from two sessions | Stop one session; reconstruct `PROGRESS.md` from `git log -- tasks/` and the latest commit messages. |
| Task stuck at `[~]` after compaction | Subagent session ended without writing `[x]` | Handoff section captures state; resume dispatches a fresh subagent with full context. |
| Plan and `PROGRESS.md` drift out of sync | Manual edits to task files bypassing the TOML | Re-run `wiggum generate`; let the orchestrator re-derive state. |
...
const CAPABILITY_TEMPLATE: &str = r#"# {{ title }}

> **Capability**: `{{ name }}` — behavioural contract the system must satisfy.
...
{% if requirements %}{% for req in requirements %}- {{ req }}
{% endfor %}{% else %}<!-- TODO: Add at least one requirement.
   Use MUST / SHALL phrasing — these are the non-negotiable contracts the
   capability must satisfy. Rendered as a flat bullet here so subagents
   can grep for the slug and find the contract. -->
{% endif %}
## Scenarios
...
- **WHEN** {{ s.when }}
- **THEN** {{ s.then }}
{% endfor %}{% else %}<!-- TODO: Add at least one WHEN/THEN scenario.
   Scenarios are machine-readable acceptance contracts — the evaluator
   parses them back out of task files to verify completion. A scenario
   without a THEN clause cannot be verified. -->
{% endif %}
---

_Generated by [wiggum](https://github.com/greysquirr3l/wiggum). This file is part of the project's behavioural specification; the requirements and scenarios here should not change without updating every `[[phases.tasks]] implements = ["{{ name }}"]` reference._
"#;
